...
class WP_Image_Size_Limit {
	...
	private $current_user_roles = null;
	...
	public function __construct()  {  
			...
			if (!empty($options['img_upload_limit_exempt'])) {
				$this->exempt_roles = $options['img_upload_limit_exempt'];
			}
	...
	public function is_exempt( $user ) {
		if (!is_object($user) || !isset($user->roles)) return false;
		$roles = $user->roles;
		foreach ($this->exempt_roles as $exempt_role) {
			if (in_array($exempt_role, $roles)) return true;
		}	
		return false;
	}
	...
	public function error_message($file) {
		...
		if ($this->is_exempt($user)) return $file;
